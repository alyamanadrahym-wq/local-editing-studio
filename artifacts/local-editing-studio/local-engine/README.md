...
A self-hosted runner labeled `Windows`, `X64`, and `RTX` is required for CUDA transcription and a real `h264_nvenc` export. Install a current NVIDIA RTX GPU and driver plus the CUDA/cuDNN runtime required by CTranslate2. The workflow provisions Python, installs the engine dependencies, downloads the same pinned FFmpeg 7.1.1 archive used by the installer, verifies its SHA-256 before extraction, and verifies `ffmpeg`, `ffprobe`, `nvidia-smi`, the reported RTX model, CUDA, and a real NVENC encode before testing. It never silently passes through the CPU fallback. The CPU job explicitly verifies `libx264` and verifies that `auto` falls back to it when the NVENC probe fails.
