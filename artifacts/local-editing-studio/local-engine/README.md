...
For NVIDIA acceleration, install a current NVIDIA driver and the CUDA/cuDNN runtime versions required by the pinned CTranslate2 package. Release qualification uses the same pinned FFmpeg 7.1.1 build bundled by the installer, verified by SHA-256, rather than an independently changing package-manager version. `/health` reports the detected GPU names, CUDA availability, whether an RTX GPU was found, and `h264_nvenc.usable`. The latter is an actual one-frame FFmpeg NVENC encode test, not just an encoder-list check. `device: "auto"` uses CUDA when CTranslate2 can access it and otherwise uses CPU. Rendering selects `h264_nvenc` only when that test passes; otherwise it uses `libx264`.
...
A self-hosted runner labeled `Windows`, `X64`, and `RTX` is required for CUDA transcription and a real `h264_nvenc` export. Install a current NVIDIA RTX GPU and driver. Release qualification pins CTranslate2 to `4.6.0`, PyAV to `13.1.0`, and provisions the required Windows cuBLAS 12.4 and cuDNN 9.1 runtime DLLs from `requirements-cuda.txt`; a separate system CUDA Toolkit installation is not required for the smoke runner. On Windows machines with Application Control, the runner policy must trust the Python environment's downloaded `.pyd` and `.dll` dependencies; if the policy uses Zone.Identifier, run `Unblock-File` on those files immediately after `pip install` (before importing `faster_whisper`). Do not disable Application Control or silently bypass a Code Integrity policy. The workflow provisions Python, installs the engine dependencies, downloads the same pinned FFmpeg 7.1.1 archive used by the installer, verifies its SHA-256 before extraction, and verifies `ffmpeg`, `ffprobe`, `nvidia-smi`, the reported RTX model, CUDA, and a real NVENC encode before testing. It never silently passes through the CPU fallback. The CPU job explicitly verifies `libx264` and verifies that `auto` falls back to it when the NVENC probe fails.
